...
fn main() {
    ...
    // FxMini runs its own WASAPI loop, and sndDevices drags in its own web of
    // dependencies.
...
fn embed_windows_resources() {
    ...
    let icon_path = out_dir.join("fxmini.ico");
    ...
    let script_path = out_dir.join("fxmini.rc");
    ...
    let Some(rc) = find_resource_compiler() else {
        println!(
            "cargo:warning=rc.exe was not found, so the binaries will have no icon or version \
             information. Set FXMINI_RC to its full path, or build with the Windows SDK on PATH."
        );
        return;
    };

    let resource_path = out_dir.join("fxmini.res");
    ...
    println!("cargo:rerun-if-env-changed=FXMINI_RC");
}
...
BEGIN
  ...
  BEGIN
    BLOCK \"040904B0\"
    BEGIN
      VALUE \"CompanyName\", \"FxMini\"
      VALUE \"FileDescription\", \"{description}\"
      VALUE \"FileVersion\", \"{version}\"
      VALUE \"InternalName\", \"fxmini\"
      VALUE \"OriginalFilename\", \"fxmini.exe\"
      VALUE \"ProductName\", \"FxMini\"
...
fn find_resource_compiler() -> Option<PathBuf> {
    if let Some(explicit) = std::env::var_os("FXMINI_RC") {
