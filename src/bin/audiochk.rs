...
//! sound card's state, and every `.fac` FxMini can see. This changes nothing
...
use fxmini::device::{self, ComGuard};
use fxmini::{driver, preset};
...
fn run_engine(seconds: f64, source: Option<String>, sink: Option<String>) {
    ...
    let mut config = fxmini::Config::default();
    ...
    let engine = match fxmini::AudioEngine::start(config) {
        Ok(engine) => engine,
        Err(err) => {
