...
//! straight past the enhancer: FxMini builds its graph, logs a healthy
...
//! off the current state: whether FxMini can take the default output over *and
...
use fxmini::config::Config;
use fxmini::device::{self, ComGuard, DeviceInfo};
use fxmini::routing::{self, EngageOutcome, Routing};
...
fn main() {
    ...
    println!("FxMini audio environment");
    println!("========================");
    ...
    match (&default, &virtual_card) {
        (Some(default), Some(card)) if default.id == card.id => {
            ...
            println!("  -> audio flows through FxMini. Enhancement can be heard.");
        }
        (Some(default), Some(card)) => {
            ...
            println!("     FxMini captures the virtual card, which receives nothing,");
...
fn restore_output(config: &Config) -> bool {
    ...
    // Exactly what `fxmini --restore-output` runs, so the two cannot disagree:
