...
//!
//! FxMini has to be running for audio to work at all: it points the system's
//! default output at the virtual sound card and drains it (see
//! [`crate::routing`]). If the machine restarts while FxMini holds the default
//! output — or the user simply logs back in — the output is still on the virtual
//! card and nothing is draining it, so the machine is silent until FxMini comes
...
impl StartupState {
    /// Whether Windows will actually launch FxMini at logon.
...
/// Whether FxMini would start at logon.
...
/// Whether the registered entry still points at *this* copy of FxMini.
///
...
pub fn apply(desired: bool) -> bool {
    match reconcile(desired) {
        ...
        Ok(Reconcile::Repaired) => {
            log::info!("startup entry pointed at another copy of FxMini; rewrote it")
        }
