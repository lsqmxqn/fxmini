...
/// invented, so a FxMini window sits at the same height as the windows around
...
pub fn resize_border(ctx: &egui::Context, maximized: bool) {
    ...
    for (index, (rect, direction)) in zones(window).into_iter().enumerate() {
        ...
        let _ = egui::Area::new(Id::new(("fxmini-resize", index)))
