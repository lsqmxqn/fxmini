...
pub enum PresetOrigin {
    ...
    BuiltIn,
    /// Dropped into `%APPDATA%\FxMini\presets` by the user.
    User,
}
...
/// Unpacks the embedded presets into `%APPDATA%\FxMini\presets`.
///
...
/// Every preset FxMini can see, sorted by display name.
///
/// One folder serves both purposes: `%APPDATA%\FxMini\presets` is where the
