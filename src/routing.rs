...
//!
//! FxMini's audio chain only works when the *system default output* is FxSound's
//! virtual card:
//!
//! ```text
...
//! virtual card receives nothing, and FxMini cheerfully runs its whole graph
...
//!   switch to their TV or headset — a deliberate act — FxMini notices, says so
...
/// Owns the difference between the machine's routing and the one FxMini needs.
///
...
impl Routing {
    ...
    pub fn release(&mut self) -> bool {
        ...
        let target = match device::device_by_id(&previous) {
            ...
            _ => {
                log::warn!(
                    "the output displaced by FxMini ('{}') is gone; picking another",
...
/// silence.
///
/// Returns whether the routing was changed. Used by `fxmini --restore-output`,
