//! Persistent settings and the well-known paths FxMini owns.
//!
//! Everything lives under `%APPDATA%\FxMini`. The file is plain JSON so a user
...
pub const APP_DIR_NAME: &str = "FxMini";
...
pub struct Config {
    ...
    /// the system default before FxMini took over".
    ...
    /// physical card, FxMini captures a device nothing is playing into and
...
/// `%APPDATA%\FxMini`, or the current directory if `APPDATA` is unset.
...
/// `%APPDATA%\FxMini\config.json`.
pub fn config_path() -> PathBuf {
    app_data_dir().join("config.json")
}

/// `%APPDATA%\FxMini\presets` — where bundled presets are unpacked and where
/// user-supplied `.fac` files are picked up.
pub fn presets_dir() -> PathBuf {
    app_data_dir().join("presets")
}

/// `%APPDATA%\FxMini\fxmini.log`.
pub fn log_path() -> PathBuf {
    app_data_dir().join("fxmini.log")
}
