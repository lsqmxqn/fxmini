...
const INSTANCE_MUTEX: &str = r"Local\FxMini.SingleInstance";
...
fn main() {
    ...
    if std::env::args().any(|arg| arg == ARG_RESTORE_OUTPUT) {
        ...
        log::info!(
            ...
            if changed {
                ...
                "nothing to undo; the default output was not pointing at the virtual sound card"
            }
        );
        return;
    }
    ...
    let _instance = match acquire_single_instance() {
        Some(guard) => guard,
        None => {
            log::warn!("FxMini is already running; exiting");
            return;
        }
    };
    ...
    // rather than a convenience: if the machine logs in without FxMini while the
    ...
    let mut app = match app::App::start(config) {
        Ok(app) => app,
        Err(err) => {
            log::error!("FxMini could not start: {err}");
            return;
        }
    };
    ...
    log::info!("FxMini running (tray icon should be visible)");
...
fn acquire_single_instance() -> Option<InstanceGuard> {
    ...
    let name = fxmini::ffi::to_wide(std::ffi::OsStr::new(INSTANCE_MUTEX));
...
/// Starts file logging under `%APPDATA%\FxMini\fxmini.log`.
///
...
fn init_logging() {
    ...
    let level = match std::env::var("FXMINI_LOG").ok().as_deref() {
