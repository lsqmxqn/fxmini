...
| M12 | 托盘语言对勾修复 | ✅ 完成（未发版）。用户实测报：在中文状态下切到英文，中文的对勾不消失（两项同时带勾）；再次点击英文，反而把它自己的对勾去掉。成因是语言条目的对勾从未被刷新，且 `set_language` 对「当前语言」提前返回，把系统已切换的对勾留在屏幕上。见[开发笔记](开发笔记.md)第 10 节 |
