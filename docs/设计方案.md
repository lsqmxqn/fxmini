...
| M11 | 窗口边缘修复（v0.1.3 续） | ✅ 完成。用户实测报「最左侧和顶部有约 2–3px 的横条」。逐像素量出是**三个成因叠加**：winit 为无边框窗口的投影标记把客户区下移 1px、`WS_EX_WINDOWEDGE` 被 winit 留下且无法清除、自绘描边 `shrink(0.5)` 把 1 pt 摊到两个像素。另修掉 egui `Panel` 默认画出的三条分隔线。见[开发笔记](开发笔记.md)第 9 节 |
