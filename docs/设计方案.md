...
| M14 | 静音时欠载虚增（v0.1.7） | ✅ 完成。用户实测报「没有音频输出的时候欠载会快速升高」。播放本身没有任何问题——坏的是计数器：WASAPI 回环捕获在系统静音时**不产生任何包**（无播放流可抓，`GetBuffer` 直接回 `AUDCLNT_S_BUFFER_EMPTY`），而渲染端是真实声卡、时钟不停、每个周期都来要一整块帧，于是空闲时每个周期都被记成整块欠载。修法是以「本周期捕获端是否真的供过数据」为判据：静音补零但不计数，捕获在跑时缺帧仍计为真丢帧。见[开发笔记](开发笔记.md)第 12 节 |
