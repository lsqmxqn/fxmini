...
| M14 | 静音时欠载虚增（v0.1.7） | ✅ 完成。用户实测反馈「在没有音频输出的情况下，欠载会快速升高」。**这不是丢音，是计数器在算错账**：WASAPI 回环捕获在系统静音时**一个包都不产生**（没有播放流可挂，`GetBuffer` 直接回 `AUDCLNT_S_BUFFER_EMPTY`，连 SILENT 标记的包都没有），而渲染端是真实声卡、时钟一直在跑、每个周期都来要数据。于是空闲的每一个周期都被记成一整块的欠载，128 帧缓冲下就是每秒上万。修法是把「捕获端本周期是否真的供过数据」作为判据：静音时补零但**不计数**，捕获在跑时缺帧仍照记为真丢帧。见第 12 节。测试 70 → 72 |
...
cargo test --lib --locked (debug)     72 passed / 1 ignored / 0 failed
cargo test --lib --release --locked   72 passed / 1 ignored / 0 failed
...
## 12. 静音时欠载虚增（M14，v0.1.7）

### 12.1 现象先分清：是丢音，还是计数算错

用户的原话是「在没有音频输出的情况下，欠载会快速升高」。收到这句先要问的是：**播放到底有没有出问题？** 这一条决定了修哪里。答案是**没有**——静音时本来就该是静音，听不出任何异常，坏的只是底部那个欠载读数在疯涨。

这个区分很重要，因为它排掉了一整类「ring 缓冲不够大 / 线程优先级不够 / 抖动太大」的思路。那些问题会让声音真的断续，而这里声音从来没断过。

### 12.2 机制：回环捕获在静音时**没有包**，渲染端却一直在要

链路两端的行为是不对称的，这是整件事的根：

- **捕获端（回环）** 探的是「系统正在播放什么」。WASAPI 只在有活动播放流的时候才往渲染端点推数据，回环抓的就是这一路。**系统一静音，就没有流可抓，`GetBuffer` 直接返回 `AUDCLNT_S_BUFFER_EMPTY`，一个包都不给**——注意不是「给一堆 SILENT 标记的包」，是真的什么都没有。这一点在微软自己的 WASAPI 文档和 NAudio 的文档里都写明了：

  > "If no audio is playing whatsoever, then the `DataAvailable` event won't fire."
  >
  > "WASAPI will only push data down to the render endpoint when there are active streams. When nothing is playing, there is nothing to capture."

- **渲染端（真实声卡）** 有自己的时钟，**不会因为静音就停**，每个事件周期都来要一整块帧。

于是空闲时是这样的：捕获端 `break` 出来，什么都不推；ring 保持空；渲染端每个周期 `pop_frames` 都拿到 0，`frames - got` 就等于整个请求块。128 帧的缓冲下，那是每秒上万帧被记成「欠载」。

代码里原来的写法看不出这个错，因为它只看结果：

```rust
let got = ring.pop_frames(frames, scratch);
if got < frames {
    status.underruns.fetch_add((frames - got) as u64, Ordering::Relaxed);
}
```

「ring 空」被当成「供不上」了。但静音时 ring 空是**应有的状态**。

### 12.3 修法：把「这一周期捕获端供过数据吗」作为判据

`pump_capture` 改成返回本周期实际推入的帧数，`pump_render` 多收一个 `capture_active`，计数判断提成一个纯函数：

```rust
fn account_underrun(missing: usize, capture_active: bool, status: &EngineStatus) {
    if missing > 0 && capture_active {
        status.underruns.fetch_add(missing as u64, Ordering::Relaxed);
    }
}
```

语义变成：**捕获端在跑，ring 却供不上 → 真丢帧，照记；捕获端本就没数据 → 静音补齐，不记。**

把判断抽成独立函数不是为了好看，是因为 `pump_render` 要一个活的 COM `Graph` 才能跑，单测建不出来；而这个判断本身才是 bug 所在，不是那堆缓冲区搬运。抽出来才测得动真正起作用的那一行。

### 12.4 假绿防线：注入验证

按这个项目的惯例，修完要反向验证一次——把守卫改回无条件，测试必须变红。结果是：

```
test engine::tests::idle_capture_does_not_accrue_underruns ... FAILED
assertion `left == right` failed: an idle graph reported underruns;
  the counter is billing silence
  left: 1280
 right: 0
```

10 个空闲周期记出 **1280** 帧（10 × 128），正好是用户看到的现象。验证完随即还原。同时保留了一条反向测试 `a_dry_ring_with_active_capture_is_still_an_underrun`，确认这个修复**没有顺手把真丢帧也一起按掉**——守卫只管静音那一路，捕获在跑时的缺帧仍然计数。
...
|---|---|
| `cargo check --all-targets --locked` | exit 0 |
| `cargo test --lib --locked` | 72 passed / 1 ignored / 0 failed |
| `cargo clippy --lib --all-targets --locked` | exit 0，无告警 |
| `cargo test --lib --release --locked` | 72 passed / 1 ignored / 0 failed |

### 12.6 发版：0.1.7

- 提交 `5734075`，附注标签 `v0.1.7`，分支与标签一次 push 送出。
- `Release / v0.1.7` 与 `CI / main` 都在 `5734075` 上 **success**，GitHub Release
  `v0.1.7` 已发布（附 `FxMini-0.1.7-win64.zip` 3,790,149 字节 + `SHA256SUMS.txt`）。
- 本机包 `FxMini-0.1.7-win64.zip` 3,707,083 字节，exe 8,499,200 字节 self-contained。
- 下载发布件实测：zip 哈希 `19d15b9c…` 与包内 `fxmini.exe` 哈希 `45b7ace3…` **都等于清单声明值**。

### 12.7 版本号约定（用户明确要求）

**以后每次代码修改都要 bump 版本号**，语义按 SemVer 的三位：
...
`Cargo.toml` 与 `Cargo.lock` 里的 `fxmini` 条目**必须同时改**——CI 与 `package.ps1 -Locked`
都走 `--locked`，不一致直接失败。同步更新 README 版本历史、本文档里程碑表与质量门测试数、
`docs/设计方案.md` 里程碑表。不要等用户提醒，改完代码就 bump。
