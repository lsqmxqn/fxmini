...
| M11 | 实测问题修复（v0.1.3 续） | ✅ 完成。用户实测反馈「最左侧和顶部有一个约 2–3px 的横条」。逐像素量出来的是**三个叠在一起的成因**，其中两条在 winit 里：无边框窗口的投影标记让客户区下移 1px、`WS_EX_WINDOWEDGE` 被 winit 留下且无法清除；第三条是自己那圈描边 `shrink(0.5)` 把 1pt 摊到了两个像素上。另有 egui `Panel` 默认的分隔线制造了三条「凭空的白/灰边界」。排查过程与结论见第 9 节。测试 61 → 64 |
...
cargo test --lib --locked (debug)     64 passed / 1 ignored / 0 failed
cargo test --lib --release --locked   64 passed / 1 ignored / 0 failed
