...
cargo test --lib --locked (debug)     53 passed / 1 ignored / 0 failed
cargo test --lib --release --locked   53 passed / 1 ignored / 0 failed
...
- 区域是 **1-bit 掩码，不做抗锯齿**。8 pt 半径下四角会有约 1 物理像素的台阶——这是 `SetWindowRgn` 的固有代价，要平滑只能整窗无边框 + 自绘，代价远大于收益；
- `SetWindowRgn` 失败时区域对象要自己删（`windows` 的 `HRGN` 实现了 `Free`，`free()` 会调 `DeleteObject`）；
- 尺寸没变就不重裁，避免每帧进一次 GDI。
