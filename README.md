...
- **修掉「没有音频输出时欠载快速升高」。** 先要分清一件事：**播放本身完全正常**，静音时本来就该是静音，坏的只是底部那个欠载读数在涨。这不是丢音，是**计数器在算错账**。
  - **链路的两个端点在静音时行为不对称，这就是根因。** 捕获端探的是「系统正在播放什么」——WASAPI 只在有活动播放流时才往渲染端点推数据，回环抓的就是这一路，所以**系统一静音，就没有流可抓，`GetBuffer` 直接返回 `AUDCLNT_S_BUFFER_EMPTY`，一个包都不给**（注意不是「给一堆带 SILENT 标记的包」，是真的什么都没有）。渲染端则是真实声卡，有自己的时钟，**不会因为静音就停**，每个事件周期都来要一整块帧。
  - 于是空闲时的每一圈都是：捕获端空手而归 → ring 保持空 → 渲染端 `pop_frames` 拿到 0 → 「缺了整整一块」被记成欠载。128 帧的缓冲下那是每秒上万帧。
  - 原代码只看结果（`if got < frames`），把「ring 空」当成了「供不上」。但**静音时 ring 空是应有的状态**。现在多传一个「本周期捕获端是否真的推过数据」的判据：静音补齐不计数，捕获在跑时缺帧仍照记为真丢帧。
  - 计数判断提成了一个独立的小函数，因为这行判断才是 bug 所在，而 `pump_render` 需要一个活的 COM 对象才能跑、单测建不出来——抽出来才测得动真正起作用的那一行。
- **反向验证过**：把守卫改回无条件，新测试立刻报红，10 个空闲周期记出 **1280** 帧（10 × 128），正是用户看到的现象；随即还原。另有一条反向测试确认这个改动**没有顺手把真丢帧也一起按掉**。
- 新增 2 个测试。测试总数 70 → **72**。
