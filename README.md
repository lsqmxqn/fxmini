...
|---|---|
| 启用音效 | 总开关，关掉即旁通 DSP |
| 输出走 FxMini | 把系统默认输出指向虚拟声卡。**已经生效时这一项会置灰**，所以"右键看看它灰没灰"就是"音频到底走了增强没有"的答案 |
| 调音面板… | 打开调音窗口（已打开时再次点击会聚焦到现有窗口） |
| 预设 ▸ | 切换 `.fac` 预设，当前生效的一项带勾。面板里新存的预设会出现在这里 |
| 开机自启 | 见上文说明 |
| 语言 ▸ | **中文** / English，点一下立即切换，选择会被记住 |
| 安装虚拟声卡… / 卸载虚拟声卡 | 会触发 UAC；不适用的那一项自动置灰 |
| 重新扫描预设 | 重新读取用户预设目录。面板里存预设是自动刷新列表的，这一项主要用于从外部丢文件进来时 |
| 退出 | 归还默认输出后退出 |

菜单文案是**单语言**的（不带中英并排），默认跟随系统语言；切换语言会重建整个菜单，各项文字、面板与托盘提示一起换。
...
右键托盘 → **调音面板…**。面板本身也是双语的。关掉即释放 GL 上下文与字体图集，不留常驻窗口。

面板最下面一节是**保存为预设**：填个名字点「保存」，当前的音效、均衡与输出设置会写成一份 `.fac`，落在 `%APPDATA%\FxMini\presets\`，随即出现在上方面板的预设列表与托盘的**预设**子菜单里——不需要手动「重新扫描预设」。重名会直接覆盖，所以调出一个满意的音色后可以再点一次保存把同一个名字刷新。
