...
FxMini reuses FxSound's package instead of producing its own.
...
`Find-DriverDir` still accepts an override — `FXMINI_DRIVER_DIR`, or a `driver/` folder
...
`LICENSE` (the same text FxSound ships alongside it). FxMini's own root `LICENSE`
(AGPL-3.0) covers the rest of this repository; the driver keeps its own terms even though
they happen to agree.

**"FxSound" and the FxSound name and artwork belong to their owner.** FxMini is an
...
signature intact — see the trademark section of `dist/FxMini/LICENSE.txt`.
